...
def prepare_prices(csv_name):
    ...
    pr_df = pd.DataFrame()
    pr_df['region_id'] = prices['short_name']
    ...
    pr_df['end_dist'] = 0;
    pr_df.loc[prices['range_end']!='\\N','end_dist'] = prices['range_end'][prices['range_end']!='\\N'].astype('int64');
...
def intervals_for_region(df,region_id):
    region_mask = df['region_id']==region_id
    intervals = np.transpose(np.array([df.loc[region_mask]['begin_dist'].unique(), df.loc[region_mask]['end_dist'].unique()]))
...
def make_working_df(df,*,target_key,add_month,diesel_file,adjust_dist,diesel_num=0):
    ...
    tmp_df['dist'] = df['distance'].to_numpy()
    if adjust_dist==True:
        tmp_df['dist'] = np.maximum(tmp_df['dist'].to_numpy(),base_dist)   
    tmp_df['target'] = df[target_key]
    tmp_df['targ_price'] = tmp_df['target'].to_numpy()/tmp_df['dist']
    tmp_df['region_id'] = df['region_code'].to_numpy()
...
def refine_df(df,t_df):
    ...
    restricted = list(set(df['region_id'].unique())-set(t_df['region_id'].unique()))
    res = exclude_key(res,'region_id',restricted)
    return res
...
def df_region_crop(df,region_id = None, crops_id = None):
    sel = dict()
    if region_id!=None:
...
def fprice_by_date_region(t_df,date,r_id):
    def price_by_dist(dist):
        t_mask = (t_df['begin_date']<=date)&(t_df['end_date']>=date)&(t_df['region_id']==r_id)
...
def region_regression_series(df,t_df,*,window,min_samples = 5,min_dists = 3,include_near = False,include_long = True,region_id = 23):
    ...
    for date in date_range:
        ...
        if (not include_near):
            sub = sub.loc[sub['dist']>base_dist]
        if (not include_long):
            sub = sub.loc[sub['dist']<=long_dist]
        if ((sub.shape[0]>=min_samples)&(len(sub['dist'].unique())>=min_dists)):
            d = sub['dist'].to_numpy()
            p = sub['target'].to_numpy()
            fun = curve_tarif_k1(t_df, date, region_id)
            par,cov = curve_fit(fun,d,p,bounds=([0.5],[2]))
            ks.loc[date,'k1'] = par[0]
    return ks
def region_regression_series_k0_k1(df,t_df,*,window,min_samples = 5,min_dists = 3,include_near = False,include_long = True,region_id = 23):
    delta = pd.Timedelta(days=window-1)
    start_date = df.index.get_level_values(0).min()+delta
    end_date = df.index.get_level_values(0).max()
    date_range = pd.date_range(start_date,end_date)
    ...
    for date in date_range:
        ...
        if ((sub.shape[0]>=min_samples)&(len(sub['dist'].unique())>=min_dists)):
            d = sub['dist'].to_numpy()
            p = sub['target'].to_numpy()
            fun = curve_tarif_k0_k1(t_df, date, region_id)
...
def filter(df,use_filter,filter_window,filter_order):
    ...
    for key in df.columns:
        if use_filter == True:
            fdf[filter_key(key)] = savgol_filter(df[key].interpolate().to_numpy(),window_length = filter_window, polyorder=filter_order)
        else:
...
def predict_series_on_date(kdf,pdf,date,*,delta, pred_start = 3,pred_end = 5,var = 'fourier',n_harm =0,period=None,deg = 4):
    columns = kdf.columns
    ...
    for key in columns:
        px,rx = predict_series(kdf.loc[pd.date_range(loc_start,loc_end),key].to_numpy(),days = pred_end,n_harm = n_harm,\
...
def moving_pred(df,tr_days,*,pred_start = 3,pred_end = 5,n_harm =0,period=None,deg = 4,var = 'fourier'):
    ...
    for date in date_range:
        predict_series_on_date(df,preds,date,delta=delta, pred_start = pred_start,pred_end = pred_end,var=var,n_harm=n_harm, period=period,deg=deg)
    return preds
...
def scores_old(true_Y,pred,d,idx,q,adjust_q,targ,main_metr):
    ...
    sc_dict = {'minE':minE,'maxE':maxE,'meanE':meanE,'mae':mae,'maxE':maxE,'rmse':rmse,'quantile_'+str(round(adq*100,0)):qua}
    if main_metr == 'None':
        return E,sc_dict
    else:
...
def scores_from_df(df,targ_col,pred_col,begin_dist,end_dist):
    mask = (df['dist']>begin_dist)&(df['dist']<=end_dist)
    masked = df.loc[mask,['dist',targ_col,pred_col]]
    sub = masked[masked[pred_col].to_numpy()!=None].dropna()
    return scores(sub[targ_col],sub[pred_col],sub['dist'],sub.index)    

def scores_from_df_old(df,targ_col,pred_col,begin_dist,end_dist,*,q = 0.9,adjust_q = False,targ = 100,main_metr = 'None'):
    mask = (df['dist']>begin_dist)&(df['dist']<=end_dist)
    masked = df.loc[mask,['dist',targ_col,pred_col]]
    sub = masked[masked[pred_col].to_numpy()!=None].dropna()
    return scores_old(sub[targ_col],sub[pred_col],sub['dist'],sub.index,q,adjust_q,targ,main_metr)
...
def disp_errors_old(err_df,postfix = ''):
    ...
    fig, ax = plt.subplots()
    ax.scatter(err_df['dist'],err_df['E'])
...
def make_mean_series(df, keys, categories):
    ...
    for date, items in df.groupby(level=0):
        for i in cat_range:
            ...
            cat_mask = (items['dist']>left) & (items['dist']<= right)
            res.loc[(date,limits[i]),'dist']=right
            for key in keys:
...
def targ_score(df,targ_key,*, targ_hor = 3, begin_dist = base_dist,end_dist = 5000, targ_metr = 'quantile_90.0'):
    ...
    return scores_from_df_old(df,targ_key,pr_key,begin_dist,end_dist,q = 0.9,adjust_q = False,targ = 100,main_metr = targ_metr)

def test_score_df_old (df,targ_key,pred_key,postfix,*,plot_results = True, begin_dist = base_dist, end_dist = 5000, q = 0.9, adjust_q = True, sep = 300, targ = [5000,10000]):
    if sep==0:
        print('Results for distances '+str(begin_dist)+'-'+str(end_dist)+' km')
    else:
        print('Results for distances < '+str(sep)+' km')
        _, sc = scores_from_df_old(df,targ_key,pred_key,begin_dist,sep,q = q,adjust_q = adjust_q,targ = targ[0],main_metr = 'None')
        print_scores_old(sc)
        print('Results for distances > '+str(sep)+' km')
        _, sc = scores_from_df_old(df,targ_key,pred_key,sep,end_dist,q = q,adjust_q = adjust_q,targ = targ[1],main_metr = 'None')
        print_scores_old(sc)
    if plot_results:
        err,_ = scores_from_df_old(df,targ_key,pred_key,begin_dist,end_dist,q = q,adjust_q = False,targ = targ[0],main_metr = 'None')
...
def interp_describe(desc):
    ...
    print('Тогда существующая ошибка лежит в интервале '+str(round(M-2*sig,0))+'..'+str(round(M+2*sig,0))+'р')
...
def test_score_h_old(df,targ_key,hor,*,plot_results = True, begin_dist = base_dist, end_dist = 5000, mean_df = pd.DataFrame(), q = 0.9, adjust_q = True, sep = 300, targ = [5000,10000]):
    pr_key = pred_key(targ_key,hor);
    print('Results for target')
    postfix = 'for target on '+str(hor)+'-days prediction'
    test_score_df_old(df,targ_key,pr_key,postfix,plot_results = plot_results, begin_dist = begin_dist, end_dist = end_dist, q = q, adjust_q = adjust_q, sep = sep, targ = targ)
    if not mean_df.empty:
        print('Results for mean target')
        postfix = 'for mean target on '+str(hor)+'-days prediction'
        test_score_df_old(mean_df,targ_key,pr_key,postfix,plot_results = plot_results, begin_dist = begin_dist, end_dist = end_dist, q = q, adjust_q = adjust_q, sep = sep, targ = targ)

def test_score_h(df,targ_key,hor,*,plot_results = True, begin_dist = base_dist, end_dist = 5000, mean_df = pd.DataFrame(), sep = 300):
    pr_key = pred_key(targ_key,hor);
    print('Results for target')
    postfix = 'for target on '+str(hor)+'-days prediction'
    test_score_df(df,targ_key,pr_key,postfix,plot_results = plot_results, begin_dist = begin_dist, end_dist = end_dist, sep = sep)
    if not mean_df.empty:
        print('Results for mean target')
        postfix = 'for mean target on '+str(hor)+'-days prediction'
        test_score_df(mean_df,targ_key,pr_key,postfix,plot_results = plot_results, begin_dist = begin_dist, end_dist = end_dist, sep = sep)


def test_score_old(df,targ_key,region_id, t_df,*,plot_results = True, begin_dist = base_dist, end_dist = 5000, pred_start = 3, pred_end = 5, calc_mean = True, q = 0.9, adjust_q = True, sep = 300, targ = [5000,10000]):
    if calc_mean:
        pr_keys = make_pred_keys([targ_key],pred_start,pred_end);
        mdf = make_mean_series(df,[targ_key]+pr_keys,intervals_for_region(t_df,region_id))
    else:
        mdf = pd.DataFrame()
    for h in range(pred_start,pred_end+1):
        print('Results for '+str(h)+'-day prediction')
        test_score_h_old(df, targ_key, h, plot_results = plot_results, begin_dist = begin_dist, end_dist = end_dist, mean_df = mdf, q = q, \
                     adjust_q = adjust_quantile, sep = sep, targ = targ)

def test_score(df,targ_key,region_id, t_df,*,plot_results = True, calc_mean = True, begin_dist = base_dist, end_dist = 5000, \
    ...
    for h in range(pred_start,pred_end+1):
        print('Results for '+str(h)+'-day prediction')
        test_score_h(df, targ_key, h, plot_results = plot_results, begin_dist = begin_dist, end_dist = end_dist, mean_df = mdf, sep = sep)
...
def filter_par_df(w_sizes,fw_sizes,forders):
    u = [True,False]
    index = pd.MultiIndex.from_product([u, w_sizes,fw_sizes,forders], names = ['use_filter', 'window', 'filter_window','filter_order'])
...
def validate_model_pars(pars):
    ...
    arima_val = (v=='arima')&(po==0)&(tw>=7)&((h>0)&(h<=4)&(p>=2*h)&(tw>=2*p)|(h==0)&(p==0))
    return poly_val|fourier_val|arima_val
    
def model_par_df(vars,train_windows,harms,periods,poly_orders):
    index = pd.MultiIndex.from_product([vars,train_windows,harms,periods,poly_orders], names = ['variant', 'train_window',\
...
def moving_prediction(fk_df,k_df,*,show_results = True, pred_start = 3, pred_end = 5, var, train_window,n_harm,period,poly_order):
    preds = moving_pred(fk_df,train_window,pred_start=3,pred_end=5,n_harm=n_harm,period = period,deg=poly_order,var=var)    
...
def restore_prediction(df,pred_df,t_df,region_id,restore_fun,keys,pred_start = 3, pred_end = 5):
    res = restore_fun(df,t_df,region_id,pred_df, make_pred_keys(make_filter_keys(keys),pred_start,pred_start),pred_key('target',pred_start))
    for h in range(pred_start+1, pred_end+1):
        res = restore_fun(res,t_df,region_id,pred_df, make_pred_keys(make_filter_keys(keys),h,h),pred_key('target',h))
    return res
    
def estimate_model(df,fdf,kdf,t_df,mpars,restore_fun,*,region_id=23,targ_hor = 3,targ_metr = 'quantile_90.0',begin_dist = base_dist,end_dist = 5000):
    var, train_window, n_harm, period, poly_order = mpars
    pred_df,fdf = moving_prediction(fdf,kdf,show_results = False,train_window = train_window,pred_start=targ_hor,pred_end=targ_hor,n_harm=n_harm,period = period,poly_order=poly_order,var=var)
    res_df = restore_prediction(df,pred_df,t_df,region_id,restore_fun,kdf.columns,pred_start = targ_hor, pred_end = targ_hor)
    _,sc = targ_score(res_df,'target', targ_hor = targ_hor, begin_dist = base_dist,end_dist = end_dist, targ_metr = 'quantile_90.0')
    return sc 
    
def test_filter_model(df,t_df,fpars,mpars,reg_fun,restore_fun,*,region_id=23,crops_id=None,pred_start = 3,pred_end = 5,sep = 300):
    ...
    var, train_window, n_harm, period, poly_order = mpars
    sel = df_region_crop(df,region_id, crops_id)
    kdf = reg_fun(sel,t_df, window = window, min_samples=min_samples, min_dists=min_dists,include_near = False,include_long = True,\
                  region_id = region_id)
    ...
    pred_df,fdf = moving_prediction(fdf,kdf,show_results = True,train_window = train_window, \
                                    pred_start=pred_start,pred_end=pred_end,n_harm=n_harm,period = period,poly_order=poly_order,var=var)
    res_df = restore_prediction(sel,pred_df,t_df,region_id,restore_fun,kdf.columns,pred_start = pred_start, pred_end = pred_end)
    test_score(res_df,'target',region_id, t_df,plot_results = True, pred_start = pred_start, pred_end = pred_end,sep = sep)
    return res_df
    
def optimize_filter_model(df,t_df,fp_df,mp_df,reg_fun,restore_fun,*,region_id=23,crops_id=None,targ_hor = 3,targ_metr = 'quantile_90.0'):
    best = {'value' : np.inf}
    ...
    for item in fp_df.iterrows():
        ...
        kdf = reg_fun(sel,t_df, window = window, min_samples=2, min_dists=2,include_near = False,include_long = True, region_id = region_id)
        fdf = filter(kdf,use_filter,filter_window,filter_order)
        ...
        for mitem in mp_df.iterrows():
            ...
            if validate_all_pars(parf,parm):
                sc = estimate_model(sel,fdf,kdf,t_df,tuple(parm),restore_fun,region_id=region_id,targ_hor = targ_hor,\
                                    targ_metr = targ_metr,begin_dist = base_dist,end_dist = 5000)
                if best['value']>sc:
                    best['value'] = sc
                    best['filter_p'] = parf
                    best['model_p'] = parm
                    print('best updated: ',sc, ' filter params '+str(ind_f+1)+'/'+str(fpar_cnt),' model params '+str(ind_m+1)+'/'+str(mpar_cnt))
    return best
...
def predict_by_precedent_dow(df, dist, date, crops_id, region_id, dest_id,tarif,dist_eps = 0.1):
    day = date.day_of_week
    #смотрим, были ли вообще перевозки на это расстояние
    dist_mask = (df['dist']==dist)
    if any(dist_mask):
    ...
    dist_mask = (df['dist']>=dist*(1-dist_eps))&(df['dist']<=dist*(1+dist_eps))
    if any(dist_mask):
        sub = df.loc[dist_mask].copy()
        #считаем дельту и сортируем в обратном порядке
        sub['delta'] = np.abs(sub['dist'].to_numpy()-dist)
    ...
    return predict_4dist_by_tarif(dist, date, region_id, tarif),'t'

#Предсказание по расстоянию, пункту назначения и культуре (в порядке приоритета)
def predict_by_precedent_crops(df, dist, date, crops_id, region_id, dest_id,tarif,dist_eps = 0.1):
    #смотрим, были ли вообще перевозки на это расстояние
    dist_mask = (df['dist']==dist)
    if any(dist_mask):
...
def predict_by_precedent_crops_dow(df, dist, date, crops_id, region_id, dest_id,tarif,dist_eps = 0.1):
    day = date.day_of_week
    #смотрим, были ли вообще перевозки на это расстояние
    dist_mask = (df['dist']==dist)
    if any(dist_mask):
    ...
    for date in date_range:
        ...
        if (loc_end+pd.Timedelta(days=3))in df.index.get_level_values(0):
            for id,item in preds.loc[loc_end+test_delta].groupby(level = 0):
                dist = item['dist'].to_numpy()[0]
                dest_id = item['dest_id'].to_numpy()[0]
                crops_id = item['crops_id'].to_numpy()[0]
                region_id = item['region_id'].to_numpy()[0]
                price,debug = fun(preds.loc[loc_start:loc_end], dist,date,crops_id,region_id,dest_id,pr_df)
...
class PrecedentPredictor(BaseEstimator):
    ...
    def __init__(self,*, tarif_df, eps = 0.1, region_id = -1):
    ...
    def __select_if(self, df):
        if (self.region_id>=0):
            return select_by_key(df,'region_id',self.region_id)
        else:
            return df.copy()        
    ...
    #на вход подаётся Pandas.DataFrame() со столбцами date,dist,dest_id,crops_id,region_id
    def predict(self,df):
        ...
        for id,item in preds.iterrows():
            ...
            price,debug = predict_by_precedent_crops(self.__train_df, dist,date,crops_id,region_id,dest_id,self.tarif_df,self.eps)
...
class SeriesPredictor(BaseEstimator):
    ...
    #region_id - код региона - обязателен, т.к. по нему определяется тариф, для которого считаются поправки
    #form_type - тип формулы 'k1' для формулы p(d)=d*k1*t(d) или 'k0k1' для формулы p(d)=k0+d*k1*t(d)
    def __init__(self,*, tarif_df, fpars, mpars,region_id, form_type, min_samples = 2, min_dists = 2):
        if not validate_all_pars(fpars,mpars):
    ...
    def __make_data(self,idx, dists):
        ...
        midx = pd.MultiIndex.from_product([date_range,dists],names = ['date', 'dist'])
        res = pd.DataFrame(index = midx,columns = ['target_pred'])
        res['dist'] = res.index.get_level_values(1)
        return res
    ...
    def fit(self, df, y=None,*, date = None):
        self.__train_df = select_by_key(df,'region_id',self.region_id)
        ...
        kdf = self.__reg_fun(self.__train_df,self.tarif_df, window = window, min_samples=self.min_samples,\
                             min_dists=self.min_dists, include_near = False,include_long = True, region_id = self.region_id)
        if not (date is None):
    ...
    def predict(self, pred_start, pred_end,*, df = None,dists = None,):
        ...
        pred_df = predict_series_once(self.__fdf,tr_days = train_window, pred_start = pred_start, pred_end = pred_end,var = var,\
        ...
        if (not df is None):
            ...
            preds = select_by_key(df.loc[min_date:max_date],'region_id',self.region_id)
        else:
            # по умолчанию, предсказываем для концов интервалов расстояний из тарифа
            if (dists is None):
                dists = intervals_for_region(self.tarif_df,self.region_id)[:,1]
            preds = self.__make_data(pred_df.index, dists)
        preds = self.__restore_fun(preds,self.tarif_df,self.region_id,pred_df,pred_df.columns,'target_pred')
        return preds
